 ...
 */
Ext.define('Assessmentapp.Application', {
	...
	requires : [ "Ext.ux.BoxReorderer", "Ext.ux.CellDragDrop",
			...
			"Ext.ProgressBarWidget", "Ext.ZIndexManager","Assessmentapp.view.databrowsercalendar.DBCalendar" ],
	...
	stores : [ 'Assessmentapp.store.QueryCriteriaWidgetsStore' ],
...
Ext.Ajax.on('requestcomplete', function(conn, response, options, eOpts) {
	...
	if (responseData.response) {
		if (responseData.response.exceptionId == 4005 && !sessionTimeOutFlag) {
			...
			var loginWindow = Ext.create('Assessmentapp.view.login.SessionLogin')
					.center();
