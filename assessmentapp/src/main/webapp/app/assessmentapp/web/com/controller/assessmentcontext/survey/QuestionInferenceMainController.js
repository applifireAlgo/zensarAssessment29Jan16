Ext.define('Assessmentapp.assessmentapp.web.com.controller.assessmentcontext.survey.QuestionInferenceMainController', {
     extend: 'Assessmentapp.view.fw.frameworkController.AggregateViewController',
     alias: 'controller.QuestionInferenceMainController',
     init: function() {
     ...
     questionIdLoad: function() {
          ...
          Ext.Ajax.request({
               ...
               success: function(response, scope) {
                    var compRef = scope.sender.down('#questionId');
                    scope.sender.controller.assignAllComboData(scope.sender, 'questionId', response.responseText);
     ...
     renderTreeGridData: function(responseData) {
          ...
          if (responseData.response.success) {
               ...
               var tree = this.view.down('#QuestionInferenceTree');
               ...
               var compRef = this.view.down('#QuestionInferenceTree');
     ...
     onDeleteActionColumnClickMainGrid: function(grid, rowIndex) {
          var recordId = grid.store.data.items[rowIndex].data.questionInferenceId;
          me = this;
     ...
     hideDataBrowser: function() {
          var form = this.view.down('#QuestionInference');
          var grid = this.view.down('#QuestionInferenceGrid');
          var tree = this.view.down('#QuestionInferenceTreeContainer');
     ...
     onFilterClick: function(but, evt) {
          ...
          Ext.Ajax.request({
               ...
               failure: function(response, currentObject) {
                    ...
                    var compRef = scope.sender.view.down('#QuestionInferenceTree');
     ...
     treeClick: function(me, record, item, index, e, eOpts) {
          if (record.data.leaf) {
               var QuestionInference = this.view.down('#QuestionInferenceForm');
               QuestionInference.reset();
               var gridPanel = this.view.down('#QuestionInferenceGrid');
          ...
          var formPanel = this.getView().up().down('#QuestionInferenceForm');
     ...
     onGridItemClick: function(me, record, item, index, e, eOpts) { /** Finding record by id */
          ...
          var treePanel = this.view.up().up().down('#QuestionInferenceTree');
     ...
     renderFormValue: function(val, me) {
          store = this.view.up().down('#QuestionInference').down('#' + me.column.dataIndex + '').store;
     ...
     setGridData: function(data) {
          var gridStore = this.view.down('#QuestionInferenceGrid').store;
          gridStore.removeAll();
          gridStore.add(data);
     ...
     loadData: function(responseData) {
          this.view.down('#QuestionInferenceForm').getViewModel().setData(responseData.response.data);
     },
     ...
     saveForm: function(but, evt) {
          ...
          var method;
          if (jsonData.questionInferenceId == null) {
               method = 'POST'
          } else {
               method = 'PUT';
               jsonData.systemInfo = {
