Ext.define('Assessmentapp.assessmentapp.web.com.controller.assessmentcontext.survey.AssessmentsMainController', {
     extend: 'Assessmentapp.view.fw.frameworkController.FrameworkViewController',
     alias: 'controller.AssessmentsMainController',
     ...
     init: function() {
          ...
          this.firstQuestionIdLoad();
     },
     firstQuestionIdLoad: function() {
          ...
          Ext.Ajax.request({
               ...
               success: function(response, scope) {
                    var compRef = scope.sender.down('#firstQuestionId');
                    scope.sender.controller.assignAllComboData(scope.sender, 'firstQuestionId', response.responseText);
     ...
     onTreeRefreshClick: function(event, toolEl, panelHeader) {
          ...
          var tree = this.view.down('#AssessmentsTree');
     ...
     refreshMainForm: function(but, data) {
          ...
          if (data != null && data instanceof Object) {
               var grid = this.view.down('#AssessmentsGrid');
               grid.store.add(data);
               var tree = this.view.down('#AssessmentsTree');
               var rootNode = tree.getRootNode();
               var childNode = {
     ...
     hideDataBrowser: function() {
          var form = this.view.down('#Assessments');
          var grid = this.view.down('#AssessmentsGrid');
          var tree = this.view.down('#AssessmentsTreeContainer');
          grid.setHidden(true);
     ...
     treeClick: function(me, record, item, index, e, eOpts) {
          if (record.data.leaf) {
               var gridPanel = this.view.up().down('#AssessmentsGrid');
               ...
               var formPanel = this.getView().up().down("#Assessments");
     ...
     onGridItemClick: function(me, record, item, index, e, eOpts) { /** Finding record by id */
          ...
          var treePanel = this.view.up().up().down('#AssessmentsTree');
     ...
     renderFormValue: function(val, me) {
          store = this.view.up().down('#Assessments').down('#' + me.column.dataIndex + '').store;
     ...
     setGridData: function(data) {
          var gridStore = this.view.down('#AssessmentsGrid').store;
          gridStore.removeAll();
     ...
     onDeleteActionColumnClickMainGrid: function(grid, rowIndex) {
          var recordId = grid.store.data.items[rowIndex].data.assessmentId;
          me = this;
     ...
     loadData: function(responseData) {
          this.view.down('#Assessments').getViewModel().setData(responseData.response.data);
     },
