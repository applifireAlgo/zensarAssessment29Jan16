Ext.define('Assessmentapp.assessmentapp.web.com.controller.assessmentcontext.survey.AssessmentAnswerMainController', {
     extend: 'Assessmentapp.view.fw.frameworkController.FrameworkViewController',
     alias: 'controller.AssessmentAnswerMainController',
     ...
     onTreeRefreshClick: function(event, toolEl, panelHeader) {
          ...
          var tree = this.view.down('#AssessmentAnswerTree');
     ...
     refreshMainForm: function(but, data) {
          ...
          if (data != null && data instanceof Object) {
               var grid = this.view.down('#AssessmentAnswerGrid');
               grid.store.add(data);
               var tree = this.view.down('#AssessmentAnswerTree');
               var rootNode = tree.getRootNode();
               var childNode = {
     ...
     hideDataBrowser: function() {
          var form = this.view.down('#AssessmentAnswer');
          var grid = this.view.down('#AssessmentAnswerGrid');
          var tree = this.view.down('#AssessmentAnswerTreeContainer');
          grid.setHidden(true);
     ...
     treeClick: function(me, record, item, index, e, eOpts) {
          if (record.data.leaf) {
               var gridPanel = this.view.up().down('#AssessmentAnswerGrid');
               ...
               var formPanel = this.getView().up().down("#AssessmentAnswer");
     ...
     onGridItemClick: function(me, record, item, index, e, eOpts) { /** Finding record by id */
          ...
          var treePanel = this.view.up().up().down('#AssessmentAnswerTree');
     ...
     renderFormValue: function(val, me) {
          store = this.view.up().down('#AssessmentAnswer').down('#' + me.column.dataIndex + '').store;
     ...
     setGridData: function(data) {
          var gridStore = this.view.down('#AssessmentAnswerGrid').store;
          gridStore.removeAll();
     ...
     onDeleteActionColumnClickMainGrid: function(grid, rowIndex) {
          var recordId = grid.store.data.items[rowIndex].data.answerId;
          me = this;
     ...
     loadData: function(responseData) {
          this.view.down('#AssessmentAnswer').getViewModel().setData(responseData.response.data);
     },
