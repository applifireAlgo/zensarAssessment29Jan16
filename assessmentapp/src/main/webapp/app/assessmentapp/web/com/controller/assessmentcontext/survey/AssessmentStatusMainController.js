Ext.define('Assessmentapp.assessmentapp.web.com.controller.assessmentcontext.survey.AssessmentStatusMainController', {
     extend: 'Assessmentapp.view.fw.frameworkController.FrameworkViewController',
     alias: 'controller.AssessmentStatusMainController',
     ...
     onTreeRefreshClick: function(event, toolEl, panelHeader) {
          ...
          var tree = this.view.down('#AssessmentStatusTree');
     ...
     refreshMainForm: function(but, data) {
          ...
          if (data != null && data instanceof Object) {
               var grid = this.view.down('#AssessmentStatusGrid');
               grid.store.add(data);
               var tree = this.view.down('#AssessmentStatusTree');
               var rootNode = tree.getRootNode();
               var childNode = {
     ...
     hideDataBrowser: function() {
          var form = this.view.down('#AssessmentStatus');
          var grid = this.view.down('#AssessmentStatusGrid');
          var tree = this.view.down('#AssessmentStatusTreeContainer');
          grid.setHidden(true);
     ...
     treeClick: function(me, record, item, index, e, eOpts) {
          if (record.data.leaf) {
               var gridPanel = this.view.up().down('#AssessmentStatusGrid');
               ...
               var formPanel = this.getView().up().down("#AssessmentStatus");
     ...
     onGridItemClick: function(me, record, item, index, e, eOpts) { /** Finding record by id */
          ...
          var treePanel = this.view.up().up().down('#AssessmentStatusTree');
     ...
     renderFormValue: function(val, me) {
          store = this.view.up().down('#AssessmentStatus').down('#' + me.column.dataIndex + '').store;
     ...
     setGridData: function(data) {
          var gridStore = this.view.down('#AssessmentStatusGrid').store;
          gridStore.removeAll();
     ...
     onDeleteActionColumnClickMainGrid: function(grid, rowIndex) {
          var recordId = grid.store.data.items[rowIndex].data.assessmentStatusId;
          me = this;
     ...
     loadData: function(responseData) {
          this.view.down('#AssessmentStatus').getViewModel().setData(responseData.response.data);
     },
     ...
     saveForm: function(but, evt) {
          ...
          var method;
          if (jsonData.assessmentStatusId == null) {
               method = 'POST'
          } else {
               method = 'PUT';
               jsonData.systemInfo = {
